...
def parse_args():
    p = argparse.ArgumentParser(
        description='TREMD basin stability / folding test for CalphaEBM'
    )
    ...
    p.add_argument('--n-replicas',   type=int,   default=4,
                   help='Number of replicas (geometric β ladder)')
    p.add_argument('--beta',  type=float, default=None,
                   help='β for target (coldest) replica (default: β=L)')
    p.add_argument('--beta-min',     type=float, default=5.0,
                   help='β for hottest replica (default: 5.0)')
    p.add_argument('--no-scale-step', action='store_true', default=False,
                   help='Disable step-size scaling by sqrt(β_target/β_k)')
...
def main():
    ...
    # ── β ladder ───────────────────────────────────────────────────────────
    from calphaebm.simulation.tremd import geometric_beta_ladder, TREMDSimulator
    ...
    if args.start_mode == 'native':
        ...
        for i in range(n_reps):
            ...
            if args.minimize:
                ...
                print(f"    Replica {i} (β={beta_ladder[i]:.1f}): minimized in {min_steps} steps  "
        ...
        for i in range(n_reps):
            ...
            if args.minimize:
                ...
                dr_nat = drmsd_to_native(R_rand, R_native, L)
                q = compute_q(R_rand, ni, nj, d0, L)
                print(f"    Replica {i} (β={beta_ladder[i]:.1f}): random (seed={seed})  "
        ...
        for i in range(n_reps):
            ...
            if args.minimize:
                ...
                print(f"    Replica {i} (β={beta_ladder[i]:.1f}): extended+minimized  "
    ...
    print(f"    Q_max={stats.get('Q_max', 0):.3f}  "
    ...
    for k, rate in enumerate(swap_rates):
        ...
        flag = ""
        if rate < 0.10: flag = "  ← TOO LOW (need more replicas or smaller β gap)"
        if rate > 0.50: flag = "  ← TOO HIGH (can remove intermediate replica)"
    ...
    out = {
        'pdb_id': pdb_id, 'chain_id': chain_id, 'L': L,
