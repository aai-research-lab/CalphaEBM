...
def compute_fes(Q, dRMSD, n_bins=20, kT=1.0, dR_range=None, smooth=1.2):
    ...
    H, Qe, dRe = np.histogram2d(
        Q, dRMSD, bins=n_bins,
        range=[(0.0, 1.0), dR_range]
    )
...
def plot_fes(fes_dir, pdb_id, out_path=None, replicas=None,
             n_bins=20, smooth=1.2, F_max=6.0):
    ...
    for idx, d in data.items():
        Q = d['Q']; dR = d['dRMSD']
        print(f"  rep{idx}: n={len(Q)}  Q=[{Q.min():.3f},{Q.max():.3f}]  "
              f"dR=[{dR.min():.2f},{dR.max():.2f}]")
    ...
    plt.subplots_adjust(left=0.12, right=0.87, bottom=0.14,
                        top=0.87, wspace=0.10)
    if n == 1:
        axes = [axes]
    ...
    for col, ((idx, d), ax) in enumerate(zip(data.items(), axes)):
        ...
        F, Qc, dRc, visited = compute_fes(
            Q, dRMSD, n_bins=n_bins,
            dR_range=(0.0, dR_max), smooth=smooth)
        ...
        # Smooth fill
        pcm = ax.pcolormesh(dRc, Qc, F_masked,
                            cmap=cmap, norm=norm,
        ...
        try:
            ax.contour(dRc, Qc, F_masked, levels=levels,
                       colors='white', linewidths=0.55, alpha=0.55)
        except Exception:
            pass

        # Native state star
        ax.plot(0.0, 1.0, '*', ms=10, color='white',
                mec='#444', mew=0.5, zorder=10, ls='none')
        ...
        ax.set_title(f'{label}{fold_tag}', fontsize=8.5, pad=4,
                     fontweight='bold' if is_target else 'normal')

        ax.text(0.97, 0.97, f'ss={g_ss:.2f} | pk={g_pk:.2f}',
                transform=ax.transAxes, fontsize=6,
                ha='right', va='top', color='white', family='monospace')
    ...
    fig.suptitle(f'{pdb_id.upper()} — HREMD free energy surfaces',
...
def main():
    ...
    args = p.parse_args()
    plot_fes(args.fes_dir, args.pdb_id, args.out,
             args.replicas, args.n_bins, args.smooth, args.f_max)
