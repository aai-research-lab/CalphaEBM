...
def parse_args():
    p = argparse.ArgumentParser(
        description='HREMD basin stability / folding test for CalphaEBM'
    )
    ...
    # Sampling
    p.add_argument('--start-mode', choices=['native', 'extended', 'random'],
                   default='random')
    p.add_argument('--step-size',  type=float, default=3e-5)
    p.add_argument('--force-cap',  type=float, default=100.0)
    p.add_argument('--beta',       type=float, default=None,
                   help='Override β (default β=L)')
    p.add_argument('--minimize',   action='store_true', default=False,
    ...
    p.add_argument('--preset',
                   choices=['small', 'medium', 'large', 'ood_small',
                             'ss_only', 'pack_only'],
                   default=None,
                   help='Use a built-in gate ladder preset')
    p.add_argument('--n-replicas', type=int, default=4,
                   help='Total replicas (1 target + n-1 hot). Ignored if --preset used.')
    p.add_argument('--ss-min',    type=float, default=0.08,
                   help='Secondary gate at hottest replica (geometric ladder)')
    p.add_argument('--pack-min',  type=float, default=0.05,
                   help='Packing gate at hottest replica (geometric ladder)')

    # HREMD settings
    p.add_argument('--n-swaps',        type=int, default=500,
                   help='Number of swap rounds')
    p.add_argument('--steps-per-swap', type=int, default=200,
                   help='MALA steps between swap attempts')
    p.add_argument('--swap-scheme',
...
def main():
    ...
    # ── Gate ladder ─────────────────────────────────────────────────────────
    from calphaebm.simulation.hremd import GateVector, HREMDSimulator, LADDER_PRESETS
    ...
    if args.preset:
        ...
        logger.info("Custom ladder: %d replicas  ss_min=%.3f  pack_min=%.3f",
                    len(gate_ladder), args.ss_min, args.pack_min)
    ...
    if args.start_mode == 'native':
        ...
        for i in range(n_reps):
            ...
            if args.minimize:
                R_rand, E_min, min_steps, min_drmsd, min_dE = minimize_structure(
                    model, R_rand, seq_batch, lengths)
        ...
        for i in range(n_reps):
            ...
            anchor[0, 0] = torch.tensor([0.0, 0.0, 0.0])
            anchor[0, 1] = torch.tensor([3.8, 0.0, 0.0])
            anchor[0, 2] = torch.tensor([3.8 + 3.8 * math.cos(2.09),
                                          3.8 * math.sin(2.09), 0.0])
            ...
            if args.minimize:
                seq_batch = seq.unsqueeze(0)
                R_ext, E_min, min_steps, min_drmsd, min_dE = minimize_structure(
                    model, R_ext, seq_batch, lengths)
                dr_nat = drmsd_to_native(R_ext, R_native, L)
                q = compute_q(R_ext, ni, nj, d0, L)
                print(f"    Replica {i}: extended+minimized in {min_steps} steps  "
    ...
    print(f"  {args.n_swaps} swap rounds × {args.steps_per_swap} steps × {n_reps} replicas  "
    ...
    print(f"    E_mean={stats.get('E_mean', 0):.3f}  "
    ...
    for k, rate in enumerate(swap_rates):
        ...
        flag = ""
        if rate < 0.10: flag = "  ← TOO LOW (ladder too spread)"
        if rate > 0.50: flag = "  ← TOO HIGH (ladder too compressed)"
        print(f"    [{k}]↔[{k+1}]  ss: {gi.secondary:.4f}→{gj.secondary:.4f}  "
    ...
    out = {
        'pdb_id': pdb_id, 'chain_id': chain_id, 'L': L, 'beta': beta,
        'start_mode': args.start_mode,
