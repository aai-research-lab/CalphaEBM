...
echo "════════════════════════════════════════════════════════"
echo "  Run49: E_Rg Flory term in packing (λ=5.0), no Rg contrastive loss"
echo "  Resume: checkpoints/run47/run1/full/step003500.pt (model only, fresh optimizer)"
echo "  Steps: 10000  |  LR: 6e-4→6e-5"
echo "════════════════════════════════════════════════════════"

calphaebm train --phase full \
    --resume checkpoints/run47/run1/full/step003500.pt --resume-model-only \
    --pdb train_entities.no_test_entries.txt \
    --steps 10000 \
    --lr 6e-4 --lr-schedule cosine --lr-final 6e-5 \
    --scalar-lr-mult 1.0 \
    --sigma-min-rad 0.05 --sigma-max-rad 2.0 \
    --packing-rg-lambda 5.0 \
    --packing-rg-r0 2.0 \
    --packing-rg-nu 0.38 \
    --lambda-rg 0.0 \
    ...
    --val-max-samples 8 \
    --validate-every 500 \
    --ckpt-dir checkpoints/run49 \
    --ckpt-every 500 \
    2>&1 | tee logs/run49.log
