...
echo "════════════════════════════════════════════════════════"
echo "  Run51: E_coord band penalty (λ=0.5) + E_Rg Flory (λ=1.0)"
echo "  Resume: checkpoints/run50/run1/full/step009000.pt (model only, fresh optimizer)"
echo "  Steps: 20000  |  LR: 6e-4→6e-5"
echo "════════════════════════════════════════════════════════"

calphaebm train --phase full \
    --resume checkpoints/run50/run1/full/step009000.pt --resume-model-only \
    ...
    --val-max-samples 8 \
    --validate-every 500 \
    --ckpt-dir checkpoints/run51 \
    --ckpt-every 500 \
    2>&1 | tee logs/run51.log
