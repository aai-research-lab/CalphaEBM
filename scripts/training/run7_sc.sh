#!/bin/bash
#SBATCH --job-name=run7_sc
#SBATCH --cpus-per-task=48
#SBATCH --gres=gpu:1
#SBATCH --time=7-00:00:00
#SBATCH --output=logs/run7_sc_%j.log
...
# ============================================================
# RUN 7: SELF-CONSISTENT TRAINING FROM SCRATCH
#
# Ultimate test: SC training alone — no full-stage warm start.
#
# Collection:  64 proteins x 10K MALA steps per round
#              save every 100 steps -> 100 snapshots/protein
#              max 32 failures per protein
#              48 workers for collection | 16 for eval watcher
#              beta ~ LogU[10, 1000] per protein
#
# Retraining:  500 steps/round (matched to 64-protein set)
#              50 rounds -> ~80% dataset coverage
#              lambda_balance ramped 1e-6 -> 0.01 over 10 rounds
#
# Losses consistent with run6 full-stage:
...
#
# Checkpoints: self-consistent/sc_round{N:03d}.pt
# Eval decoupled — run7_eval_watcher.sh as separate SLURM job.
# ============================================================
...
echo "========================================"
echo "  RUN 7: SELF-CONSISTENT TRAINING FROM SCRATCH"
echo "  PackingEnergy v6 + dRMSD-funnel"
echo "  64 proteins x 10K steps | 48 workers"
echo "  ${N_ROUNDS} rounds x 500 retrain steps"
echo "  beta ~ LogU[10, 1000] | balance ramp 1e-6->0.01"
echo "  Training from scratch — no resume"
echo "  Eval decoupled (run7_eval_watcher.sh)"
echo "  Started: $(date)"
echo "========================================"
...
for ROUND in $(seq 1 $N_ROUNDS); do
    ...
    echo ""
    echo "[run7] Round ${ROUND}/${N_ROUNDS} (resume_round=${RESUME_ROUND}) — $(date)"
    echo ""
    ...
    echo "[run7] Round ${ROUND} done — $(date)"
done

echo "========================================"
echo "  RUN 7 SC COMPLETE — $(date)"
echo "========================================"
