...
# ════════════════════════════════════════════════════════════════
#  Run54: Gentle α-Augmented Bidirectional DSM
#
#  Resume from run52 step 3500 (proven peak checkpoint).
#
#  Key changes vs run53:
#    - NARROWER α RANGE: U(0.85, 1.10) instead of U(0.65, 1.25)
#      → run53's wide range (0.65-1.25) destabilized the model:
#        crambin compacted to 60% Rg, trp-cage swelled to 132%
#      → gentle range provides bidirectional signal without
#        training on pathologically distorted structures
#    - Same ELT losses as run52 (proven configuration)
#    - Same λ_Rg = 0.1 (analytical safety net, not the fix)
#
#  Expected: ~3× slower per step than run52 due to 3 DSM samples
# ════════════════════════════════════════════════════════════════

echo "════════════════════════════════════════════════════════"
echo "  Run54: Gentle α-Augmented Bidirectional DSM"
...
calphaebm train --phase full \
    ...
    \
    `# ── Checkpoints ──` \
    --ckpt-dir checkpoints/run54 \
    --ckpt-every 500 \
    \
    2>&1 | tee logs/run54.log
