...
class GateVector:
    ...
    @staticmethod
    def ladder(n_hot: int,
               ss_min: float = 0.08,
               pack_min: float = 0.05) -> List['GateVector']:
        """
    ...
    def __repr__(self):
        return (f"GateVector(local={self.local:.2f}, rep={self.repulsion:.2f}, "
...
class HREMDSimulator:
    ...
    def __init__(
        ...
        logger.info("HREMDSimulator: %d replicas  β=%.1f  steps/swap=%d  scheme=%s",
                    self.n, beta, n_steps_per_swap, swap_scheme)
        logger.info("  Gate ladder:")
        for i, g in enumerate(gate_ladder):
            tag = "  ← TARGET" if i == 0 else ""
            logger.info("    [%d] local=%.2f  rep=%.2f  ss=%.4f  pack=%.4f%s",
                        i, g.local, g.repulsion, g.secondary, g.packing, tag)
    ...
    def initialize(self, start_mode: str = 'random',
        ...
        if R_native is not None:
            ...
            logger.info("  Native energies per replica: %s",
                        "  ".join(f"[{i}]={e:.3f}" for i, e in
                                  enumerate(self._E_native_per_replica)))
        else:
            ...
            for i, st in enumerate(structs):
                ...
                if (Q == Q) and (dR == dR):   # not nan
                    ...
                    if is_folded and not was_folded:
                        tag = " *** TARGET ***" if i == 0 else ""
                        logger.info(
                            "  *** FOLDED [replica %d] at step %d! "
                            "Q=%.3f dRMSD=%.2fÅ RMSD=%.2fÅ%s ***",
                            i, self.global_step, Q, dR, st['RMSD'], tag
                        )
    ...
    def _swap(self, i: int, j: int, term_cache: List[Dict]) -> bool:
        ...
        delta_U = self.beta * sum(
            (gj[t] - gi[t]) * (ti.get(t, 0.0) - tj.get(t, 0.0))
            for t in _TERMS
        )
    ...
    def save_fes(self, output_dir: str, pdb_id: str = 'protein'):
        ...
        for i, traj in enumerate(self._replica_traj):
            ...
            np.savez(fname,
                Q=Qs, dRMSD=dRs, RMSD=RMSDs, E=Es,
                Rg_pct=Rgs, step=steps,
    ...
    def _structural_metrics(self, R: torch.Tensor) -> Dict[str, float]:
        ...
        nan = float('nan')
        out = {'Q': nan, 'RMSD': nan, 'dRMSD': nan, 'Rg': nan,
               'Rg_pct': nan}
        ...
        pd_cur = _pdist(rc); pd_nat = _pdist(rn)
    ...
    def _log(self, term_cache: List[Dict], structs: List[Dict], elapsed: float):
        ...
        logger.info("  HREMD | step=%7d | round=%4d | elapsed=%.0fs",
                    self.global_step, self.swap_round, elapsed)
        # header
        logger.info(
            "  %-5s %-10s %-8s %-8s %-8s %-8s %-8s %-6s %-6s %-6s %-5s %-7s %-7s %-8s %-7s",
            "Rep","E_gated","ΔE","local","rep","ss","pack",
            "Q","RMSD","dRMSD","Rg%","g_ss","g_pk","swap→","MALA%")
