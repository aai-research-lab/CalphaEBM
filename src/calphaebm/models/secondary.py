...
from calphaebm.models.hbond import (
    HBondHelix,
    HBondSheet,
    HELIX_BASIN_IDX,
    SHEET_BASIN_IDX,
...
class BasinPotential(nn.Module):
    ...
    def __init__(self, hist_path: Path, theta_edges_path: Path, phi_edges_path: Path,
                 smooth_sigma: float = 2.0, pseudocount: float = 1e-6, smooth: bool = True):
        super().__init__()
        ...
        if is_prob_like:
            if smooth:
                try:
                    ...
                    hist = gaussian_filter(hist, sigma=(smooth_sigma, smooth_sigma),
            ...
            if smooth:
                try:
                    from scipy.ndimage import gaussian_filter
                    energy = gaussian_filter(energy, sigma=(smooth_sigma, smooth_sigma),
                                              mode=("nearest", "wrap")).astype(np.float32)
                except ImportError:
                    pass
...
class SecondaryStructureEnergy(nn.Module):
    ...
    def __init__(
        ...
        **legacy_kwargs,
        ...
        if legacy_kwargs:
            logger.warning("SecondaryStructureEnergy: unexpected kwargs ignored: %s",
                           list(legacy_kwargs.keys()))

        self.physics_prior = bool(physics_prior)
        if getattr(self, 'physics_prior', False):
            logger.info("SecondaryStructureEnergy: physics_prior=True — "
                        "logits forced to 0, responsibilities = softmax(-U_k)")
        ...
        self.basin_potentials = nn.ModuleList([
            BasinPotential(hist_path=basin_paths[k],
                           theta_edges_path=theta_edges_path,
                           phi_edges_path=phi_edges_path)
    ...
    def _mixture_ramachandran(self, theta_deg, phi_deg, context):
        ...
        if getattr(self, 'physics_prior', False):
            ...
            logits = torch.zeros(
                context.shape[0], context.shape[1], self.num_basins,
    ...
    def _compute_energy_components(self, theta, phi, seq, R, debug=False):
        ...
        # 1) E_ram
        E_ram_per_pos, responsibilities = self._mixture_ramachandran(
            theta_for_phi, phi_aligned, ctx)
