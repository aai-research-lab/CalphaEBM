...
from calphaebm.training.balancing import recommend_lambdas, term_forces
...
def run(args):
    ...
    ss = SecondaryStructureEnergy(
        num_aa=NUM_AA,
        emb_dim=EMB_DIM,
    ).to(device) if "secondary" in terms_set else None
    pack = PackingEnergy(
        num_aa=NUM_AA,
        emb_dim=EMB_DIM,
    ).to(device) if "packing" in terms_set else None
    ...
    if args.apply:
        ...
        print("\nCommand-line flags:")
        print(f"  --lambda-local {rec.get('local', 1.0):.6f} "
