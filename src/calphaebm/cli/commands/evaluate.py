...
def add_parser(subparsers):
    ...
    parser.add_argument(
        "--mode", choices=["traj", "basin", "watch"], default="traj",
        help=(
    ...
    parser.add_argument("--traj",
                        help="Trajectory directory (required for --mode traj)")
    parser.add_argument("--ref-pt",
                        help="Reference .pt file (default: snapshot_0000.pt)")
    parser.add_argument("--ref-xyz",
                        help="Reference .xyz file (alternative to --ref-pt)")
    parser.add_argument("--burnin", type=int, default=0,
                        help="Frames to discard as burn-in (default: 0)")
    parser.add_argument("--no-plots", action="store_true",
                        help="Skip generating plots")

    # ── Basin mode (--mode basin) ─────────────────────────────────────
    parser.add_argument("--checkpoint", default=None,
                        help="Model checkpoint (required for --mode basin)")
    parser.add_argument("--pdb", nargs="+", default=None,
                        help=(
                            "PDB IDs, file of IDs, or val-list file. "
                            "Basin: ID list or file. Watch: file path (e.g. val_hq.txt)."
                        ))
    parser.add_argument("--minimize-steps", type=int, default=0,
                        help="L-BFGS minimization steps before Langevin (default: 0)")
    ...
    parser.add_argument("--step-size", type=float, default=1e-4,
                        help="Langevin step size (default: 1e-4)")
    parser.add_argument("--force-cap", type=float, default=100.0,
                        help="Force clipping threshold (default: 100.0)")
    parser.add_argument("--out-dir", default=None,
                        help="Output directory for basin results (default: basin_results/)")
    ...
    parser.add_argument("--no-early-stop", action="store_true",
                        help="Always run all Langevin steps (no early stopping)")
    ...
    parser.add_argument("--ckpt-dir", default=None,
                        help="Checkpoint root (required for --mode watch, "
                             "e.g. checkpoints/run6)")
    parser.add_argument("--ckpt-prefix", default=None,
                        help="Checkpoint prefix (required for --mode watch, e.g. run6)")
    parser.add_argument("--max-rounds", type=int, default=10,
                        help="Stop after this many rounds (default: 10)")
    parser.add_argument("--start-round", type=int, default=1,
                        help="First round to watch (default: 1)")
    parser.add_argument("--poll-interval", type=int, default=60,
                        help="Seconds between checkpoint polls (default: 60)")

    # ── Shared eval flags (basin + watch) ────────────────────────────
    parser.add_argument("--beta", type=float, nargs="+", default=[100.0],
                        help="Inverse temperature(s). "
                             "Basin: sweep over list. Watch: uses first value. "
                             "(default: 100.0)")
    parser.add_argument("--n-steps", type=int, default=10000,
                        help="MALA/Langevin steps per structure (default: 10000)")
    parser.add_argument("--n-samples", type=int, default=64,
                        help="Structures to evaluate per round (default: 64)")
    parser.add_argument("--sampler", default="mala",
                        choices=["mala", "langevin"],
                        help="Sampling algorithm (default: mala)")
