...
def add_parser(subparsers) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--processed-cache-dir", default="./processed_cache",
    ...
    parser.add_argument(
        "--max-rg-ratio", type=float, default=1.3,
        help="Maximum Rg/Rg_Flory ratio (default: 1.3). Set to 0 to disable.",
    )

    # Output
    parser.add_argument(
        "--out", default="train_entities_curated.txt",
    ...
    parser.add_argument(
        "--require-monomer-assembly", action="store_true", default=True,
    ...
    parser.add_argument(
        "--no-monomer-assembly", action="store_false", dest="require_monomer_assembly",
        help="Disable biological assembly filter (keep multimers).",
    )
    parser.add_argument(
        "--assembly-delay", type=float, default=0.05,
...
def run(args) -> int:
    ...
    if exclude_ids:
        before = len(pdb_ids)
        pdb_ids = [pid for pid in pdb_ids
                    if pid.split("_")[0].lower() not in exclude_ids]
        logger.info("Excluded %d test set entries: %d → %d",
                     before - len(pdb_ids), before, len(pdb_ids))
    ...
    if args.require_monomer_assembly:
        ...
        before = len(pdb_ids)
        pdb_ids = mono_ids
        logger.info("Assembly filter: %d → %d (removed %d multimers)",
                     before, len(pdb_ids), len(multi_ids))
    ...
    for i in range(n_total):
        ...
        ratio = rg / rg_flory
        rg_data.append({
            "pdb_id": pid, "chain_id": cid, "L": L,
            "rg": float(rg), "rg_flory": float(rg_flory), "ratio": float(ratio),
        })
    ...
    if args.require_monomer_assembly:
        logger.info("  After assembly filter:   %d  (removed %d multimers)",
                     len(pdb_ids), len(multimer_ids_removed))
    ...
    for cutoff in [1.1, 1.2, 1.3, 1.4, 1.5, 2.0]:
        ...
        logger.info("    ≤ %.1f: %5d / %d  (%5.1f%%)%s",
                     cutoff, n, len(ratios), pct, marker)
    ...
    logger.info("")
    logger.info("  With --max-rg-ratio %.1f:  %d chains pass (%d rejected)",
                 max_rg_ratio or 999, n_pass, n_total - n_pass)
    ...
    logger.info("")
    logger.info("  Rg/Rg_Flory (passing chains): mean=%.3f  std=%.3f  "
                 "p05=%.3f  p50=%.3f  p95=%.3f",
                 np.mean(passing_ratios), np.std(passing_ratios),
    ...
    logger.info("  Train/val split (%.0f/%.0f): ~%d train + ~%d val",
                 train_pct, 100 - train_pct,
                 int(n_pass * train_pct / 100), int(n_pass * (100 - train_pct) / 100))
    ...
    for d in sorted_by_ratio[:15]:
        logger.info("    %s_%s  L=%3d  Rg=%6.1f  expected=%5.1f  ratio=%.2f",
                     d["pdb_id"], d["chain_id"], d["L"],
                     d["rg"], d["rg_flory"], d["ratio"])
    ...
    passing_entries = sorted({
        rg_data[i]["pdb_id"]
        for i in range(len(rg_data)) if mask[i]
    })
    ...
    val_path = Path(args.out_val) if args.out_val else out_path.with_name(
        out_path.stem.replace("train", "val") + out_path.suffix
        if "train" in out_path.stem else out_path.stem + "_val" + out_path.suffix)
    with open(val_path, "w") as f:
        for eid in val_entries:
            f.write(eid + "\n")
    logger.info("Saved %d val IDs to %s (%.0f/%.0f split, seed=%d)",
                len(val_entries), val_path, args.train_percent,
                100 - args.train_percent, args.seed)
    ...
    meta = {
        ...
        "length_distribution": {
            f"L{lo}-{hi}": int(((filtered_lengths >= lo) & (filtered_lengths <= hi)).sum())
            for lo, hi in bins
        },
        "filters_applied": [
            ...
            "monomer_assembly: RCSB biological assembly API" if args.require_monomer_assembly else "monomer_assembly: disabled",
