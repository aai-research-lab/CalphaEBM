...
def build_parser(subparsers):
    ...
    parser.add_argument("--max-rounds",       type=int,   default=_T["max_rounds"],    help="Max training rounds (stage full, default: 8)")
    parser.add_argument("--steps-per-round",  type=int,   default=3000,  help="Training steps per round (stage full, default: 3000)")
    parser.add_argument("--n-decoys",         type=int,   default=_T["n_decoys"],    help="IC-noised decoys per protein per step (stage full, default: 8)")
    parser.add_argument("--disc-T",           type=float, default=_T["disc_T"],   help="Temperature for discrimination loss (default: 2.0)")
    parser.add_argument("--lambda-qf",        type=float, default=_T["lambda_qf"],   help="Weight for Q-funnel loss (stage full, default: 1.0)")
    parser.add_argument("--converge-q",       type=float, default=_T["converge_q"],  help="Convergence Q threshold (default: 0.95)")
    parser.add_argument("--converge-rmsd",    type=float, default=_T["converge_rmsd"],   help="Convergence RMSD threshold in A (default: 5.0)")
    parser.add_argument("--converge-rg-lo",   type=float, default=_T["converge_rg_lo"],  help="Convergence Rg%% lower bound (default: 95)")
    parser.add_argument("--converge-rg-hi",   type=float, default=_T["converge_rg_hi"], help="Convergence Rg%% upper bound (default: 105)")
    parser.add_argument("--sigma-min-ic",     type=float, default=0.0524,  help="Min IC noise sigma in radians (stage full, default: 0.05)")
    parser.add_argument("--sigma-max-ic",     type=float, default=1.0472,   help="Max IC noise sigma in radians (stage full, default: 1.5)")
    parser.add_argument("--decoy-every",      type=int,   default=1,     help="Compute decoy losses every N steps (default: 1)")
    parser.add_argument("--val-proteins",     type=int,   default=_T["eval_proteins"],    help="Number of proteins for basin eval (default: 64)")
    parser.add_argument("--val-steps",        type=int,   default=5000,  help="Langevin steps for basin eval (default: 5000)")
    parser.add_argument("--val-beta",         type=float, default=_T["eval_beta"], help="Inverse temperature for basin eval (default: 100)")
    parser.add_argument("--T-funnel",         type=float, default=_T["T_funnel"],   help="Temperature for funnel losses (default: 2.0)")

    parser.add_argument("--train-pdb", type=str, default=None,
                        help="File with training PDB IDs (one per line). If set with --val-pdb, "
                             "overrides --pdb and uses explicit train/val split.")
    parser.add_argument("--val-pdb",   type=str, default=None,
    ...
    parser.add_argument("--processed-cache-dir", type=str, default="./processed_cache",
    ...
    parser.add_argument(
        ...
        default=0.02,
        help=(
            "Min sigma for θ (bond angle) noise (radians). "
            "θ std ≈ 0.295 rad in PDB data. Default: 0.02."
        ),
    )
    parser.add_argument(
        "--sigma-max-theta",
        ...
        default=0.5,
        help=(
            "Max sigma for θ (bond angle) noise (radians). "
            "0.5 rad ≈ 1.7× natural variation. Default: 0.5."
        ),
    )
    parser.add_argument(
        "--sigma-min-phi",
        ...
        default=0.05,
        help=(
            "Min sigma for φ (dihedral) noise (radians). "
            "φ std ≈ 1.83 rad in PDB data. Default: 0.05."
        ),
    )
    parser.add_argument(
        "--sigma-max-phi",
        ...
        default=1.5,
        help=(
            "Max sigma for φ (dihedral) noise (radians). "
            "1.5 rad covers most of the torsion landscape. Default: 1.5."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=1.25,
        help=(
            "Max α for DSM Rg scaling augmentation. "
            "Set both to 1.0 to disable augmentation. Default: 1.25."
        ),
    )

    # Validation options
    parser.add_argument("--validate-every",    type=int,   default=500,  help="Run validation every N steps")
    parser.add_argument("--val-max-samples",   type=int,   default=256,
                        help="Max structures during training validation (default: 256)")
    parser.add_argument("--val-langevin-steps",type=int,   default=500,
                        help="IC Langevin steps per structure during validation (default: 500)")
    parser.add_argument("--val-step-size",     type=float, default=None,
                        help="IC Langevin step size for validation (default: 1e-4)")
    parser.add_argument("--langevin-beta",     type=float, default=1.0,
    ...
    # ============================================================
    parser.add_argument("--init-theta-theta-weight", type=float, default=1.0,
                        help="Initial weight for theta-theta coupling (default: 1.0)")
    parser.add_argument("--init-delta-phi-weight",   type=float, default=1.0,
                        help="Initial weight for delta-phi potential (default: 1.0)")

    # ============================================================
    # GATE OVERRIDE OPTIONS
    # Applied AFTER checkpoint load.
    # ============================================================
    parser.add_argument("--set-gate-local",     type=float, default=None,
                        help="Override g_local gate after checkpoint load")
    parser.add_argument("--set-gate-secondary", type=float, default=None,
                        help="Override g_secondary gate after checkpoint load")
    parser.add_argument("--set-gate-repulsion", type=float, default=None,
                        help="Override g_repulsion gate after checkpoint load")
    parser.add_argument("--set-gate-packing",   type=float, default=None,
                        help="Override g_packing gate after checkpoint load")
    ...
    parser.add_argument("--packing-init-from",   type=str,   default="log_oe",
                        choices=["log_oe", "pmi", "pmi_signed", "zeros"])
    ...
    parser.add_argument("--packing-geom-calibration", type=str, default="analysis/packing_analysis/data/geometry_feature_calibration.json",
    ...
    parser.add_argument("--packing-rg-lambda",     type=float, default=_M["rg_lambda"],
    ...
    parser.add_argument("--packing-rg-r0",         type=float, default=_M["rg_r0"],
                        help="Flory prefactor: Rg_expected = r0 * L^nu. Default 2.0 Å.")
    parser.add_argument("--packing-rg-nu",         type=float, default=_M["rg_nu"],
                        help="Flory exponent for globular proteins. Default 0.38.")
    parser.add_argument("--hp-penalty-lambda",      type=float, default=_M["coord_lambda"],
                        help="Coordination penalty coefficient (0=disabled). "
                             "E_hp_pen = λ · (1/L) · Σ_i penalty(n_i). "
                             "Default 0.1.")
    parser.add_argument("--coord-n-star-file",      type=str,   default="coord_n_star.json",
    ...
    parser.add_argument("--packing-rg-dead-zone",  type=float, default=_M["rg_dead_zone"],
                        help="Rg penalty dead zone: no penalty within ±δ of Rg*. Default 0.30 (±30%%).")
    parser.add_argument("--packing-rg-m",          type=float, default=_M["rg_m"],
                        help="Rg penalty saturation level. Default 1.0.")
    parser.add_argument("--packing-rg-alpha",      type=float, default=_M["rg_alpha"],
                        help="Rg penalty steepness. Default 3.0.")
    parser.add_argument("--packing-coord-m",       type=float, default=_M["coord_m"],
                        help="Coordination penalty saturation level. Default 1.0.")
    parser.add_argument("--packing-coord-alpha",   type=float, default=_M["coord_alpha"],
                        help="Coordination penalty steepness. Default 2.0.")
    # Run5: contact density (ρ) energy
    parser.add_argument("--packing-rho-lambda",    type=float, default=_M["rho_lambda"],
                        help="Contact density ρ reward scale. Default 0.1.")
    parser.add_argument("--packing-rho-sigma",     type=float, default=_M["rho_sigma"],
                        help="Contact density ρ Gaussian width. Default 0.7.")
    parser.add_argument("--packing-rho-m",         type=float, default=_M["rho_m"],
                        help="Contact density ρ penalty saturation. Default 1.0.")
    parser.add_argument("--packing-rho-alpha",     type=float, default=_M["rho_alpha"],
                        help="Contact density ρ penalty steepness. Default 2.0.")
    parser.add_argument("--rho-penalty-lambda",     type=float, default=_M["rho_penalty_lambda"],
                        help="Contact density ρ penalty scale. Default 0.1.")
    # Run5: saturating exponential margins
    parser.add_argument("--funnel-m",              type=float, default=_T["funnel_m"],
                        help="Q/Rg funnel margin saturation level. Default 5.0.")
    parser.add_argument("--funnel-alpha",          type=float, default=_T["funnel_alpha"],
                        help="Q/Rg funnel margin steepness. Default 5.0.")
    parser.add_argument("--gap-m",                 type=float, default=_T["gap_m"],
                        help="Gap loss margin saturation level. Default 5.0.")
    parser.add_argument("--gap-alpha",             type=float, default=_T["gap_alpha"],
                        help="Gap loss margin steepness. Default 5.0.")
    parser.add_argument("--lambda-drmsd",          type=float, default=_T["lambda_drmsd"],
    ...
    parser.add_argument(
        ...
        default=0.5,
        help=(
            "φ perturbation sigma for IC geogap loss (radians). Default: 0.5. "
            "Tests delta_phi_energy discrimination."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument("--geogap-diag-every", type=int, default=200,
                        help="Log geogap diagnostics every N steps. Default: 200.")
    ...
    parser.add_argument(
        ...
        default=2.0,
        help=(
            "Temperature for Q-funnel exp-decay loss. "
            "Larger = gentler penalty for anti-funnel slopes. Default: 2.0."
        ),
    )
    parser.add_argument(
        "--funnel-n-decoys",
    ...
    parser.add_argument(
        ...
        default=512,
        help=(
            "Maximum chain length for ELT full-chain batches. "
            "Chains longer than this are skipped. Default: 512."
        ),
    )
    parser.add_argument(
        "--elt-batch-size",
    ...
    parser.add_argument("--learn-packing-coords",    action="store_true",
                        help="Make n_star, sigma in packing learnable (200 params)")
    parser.add_argument("--learn-packing-density",   action="store_true",
                        help="Make rho_fit_a/b/c, rho_sigma learnable (20 params)")
    parser.add_argument("--learn-penalty-shapes",    action="store_true",
                        help="Make dead_zone, m, alpha learnable (9 params)")
    parser.add_argument("--learn-packing-bounds",    action="store_true",
                        help="Make coord/rho lo/hi bounds learnable (210 params)")
    parser.add_argument("--learn-penalty-strengths", action="store_true",
                        help="Make rg/coord/rho lambda strengths learnable (3 params)")
    parser.add_argument("--learn-gate-geometry",     action="store_true",
                        help="Make rama gate basin centers/widths learnable (10 params)")
    parser.add_argument("--learn-hbond-geometry",    action="store_true",
                        help="Make hbond mu/sigma distance params learnable (6 params)")
    parser.add_argument("--learn-all-buffers",       action="store_true",
                        help="Enable ALL learnable buffer flags (~462 params)")
    ...
    sc.add_argument("--n-rounds", type=int, default=_D["n_rounds"],
                    help=f"Number of collect→retrain rounds (default: {_D['n_rounds']})")
    sc.add_argument("--collect-proteins", type=int, default=_D["collect_proteins"],
                    help=f"Proteins to randomly sample per round (default: {_D['collect_proteins']})")
    sc.add_argument("--collect-steps", type=int, default=_D["collect_steps"],
                    help=f"Langevin steps per protein during collection (default: {_D['collect_steps']})")
    sc.add_argument("--collect-beta", type=float, default=_D["collect_beta"],
                    help=f"Inverse temperature for collection dynamics (default: {_D['collect_beta']})")
    sc.add_argument("--collect-step-size", type=float, default=_D["collect_step_size"],
                    help=f"Langevin step size for collection (default: {_D['collect_step_size']})")
    sc.add_argument("--collect-save-every", type=int, default=_D["collect_save_every"],
                    help=f"Check for failures every N Langevin steps (default: {_D['collect_save_every']})")
    sc.add_argument("--n-workers", type=int, default=_D["collect_n_workers"],
    ...
    sc.add_argument("--collect-max-len", type=int, default=_D["collect_max_len"],
                    help=f"Maximum chain length for collection proteins (default: {_D['collect_max_len']})")
    sc.add_argument("--retrain-steps", type=int, default=_D["retrain_steps"],
                    help=f"Training steps per SC round (default: {_D['retrain_steps']})")
    sc.add_argument("--retrain-lr", type=float, default=_D["retrain_lr"],
                    help=f"Learning rate for SC retraining (default: {_D['retrain_lr']})")
    # Model-sampled negative losses (3× Synthetic counterparts)
    sc.add_argument("--lambda-sampled-hsm", type=float, default=_D["lambda_sampled_hsm"],
                    help=f"Sampled HSM loss weight: HSM on Langevin-sampled negatives (default: {_D['lambda_sampled_hsm']})")
    sc.add_argument("--lambda-sampled-qf", type=float, default=_D["lambda_sampled_qf"],
                    help=f"Sampled Q-funnel loss weight: 3× Synthetic QF (default: {_D['lambda_sampled_qf']})")
    sc.add_argument("--lambda-sampled-drmsd-funnel", type=float, default=_D["lambda_sampled_drmsd_funnel"],
    ...
    sc.add_argument("--lambda-sampled-gap", type=float, default=_D["lambda_sampled_gap"],
                    help=f"Sampled gap loss weight: 3× Synthetic gap (default: {_D['lambda_sampled_gap']})")
    sc.add_argument("--sc-margin", type=float, default=_D["sc_margin"],
                    help=f"Sampled gap margin in E/res units (default: {_D['sc_margin']})")
    sc.add_argument("--sc-eval-steps", type=int, default=_D["eval_steps"],
                    help=f"Langevin steps for SC basin evaluation (default: {_D['eval_steps']})")
    sc.add_argument("--sc-eval-beta", type=float, default=_D["eval_beta"],
                    help=f"Inverse temperature for SC basin evaluation (default: {_D['eval_beta']})")
    sc.add_argument("--sc-eval-proteins", type=int, default=_D["eval_proteins"],
                    help=f"Proteins for SC basin evaluation (default: {_D['eval_proteins']})")
    sc.add_argument("--rg-compact", type=float, default=_D["rg_compact"],
                    help=f"Rg ratio below this → compacted failure (default: {_D['rg_compact']})")
    sc.add_argument("--rg-swollen", type=float, default=_D["rg_swollen"],
                    help=f"Rg ratio above this → swollen failure (default: {_D['rg_swollen']})")
    sc.add_argument("--q-false-basin", type=float, default=_D["q_false_basin"],
                    help=f"Q below this + E < E_native → false basin (default: {_D['q_false_basin']})")
    sc.add_argument("--rmsd-drift", type=float, default=_D["rmsd_drift"],
                    help=f"RMSD above this + Q > 0.9 → drift failure (default: {_D['rmsd_drift']})")
    sc.add_argument("--rmsf-frozen", type=float, default=_D["rmsf_frozen"],
                    help=f"RMSF below this → frozen/over-stabilized (default: {_D['rmsf_frozen']})")
    sc.add_argument("--ss-change-thr", type=float, default=_D["ss_change_thr"],
                    help=f"SS change above this → SS loss failure (default: {_D['ss_change_thr']})")
    sc.add_argument("--max-negatives-per-protein", type=int, default=_D["max_negatives_per_protein"],
                    help=f"Cap negatives per protein (default: {_D['max_negatives_per_protein']})")
    sc.add_argument("--convergence-threshold", type=float, default=_D["convergence_threshold"],
                    help=f"Stop if E_delta improvement < this between rounds (default: {_D['convergence_threshold']})")
    sc.add_argument("--min-negatives", type=int, default=_D["min_negatives"],
                    help=f"Skip retraining if fewer negatives collected (default: {_D['min_negatives']})")
    sc.add_argument("--sc-resume-round", type=int, default=0,
                    help="Resume from this round number (default: 0 = start fresh)")
