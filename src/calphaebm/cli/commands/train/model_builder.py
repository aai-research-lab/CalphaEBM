...
def build_model(terms_set: Set[str], device: torch.device, args) -> torch.nn.Module:
    ...
    logger.debug(
        ...
        include_repulsion, include_secondary, include_packing,
    )
    ...
    if coord_n_star_file is not None:
        ...
        if _nstar_path.exists():
            ...
            logger.info("Loaded coord stats from %s  (style=%s, n_groups=%d)",
                        coord_n_star_file, _style, _n_grps)
            ...
            if _ng_mean is not None:
                ...
                logger.info("  n_group_mean [20×5]: loaded (range %.2f – %.2f)",
            ...
            if _ng_std is not None:
                ...
                logger.info("  n_group_std  [20×5]: loaded (range %.2f – %.2f, "
                            "%d/%d entries will be clamped to 0.8)",
                            min(_flat_std), max(_flat_std),
                            n_clamped, len(_flat_std))
            else:
            ...
            if _ng_lo is not None and _ng_hi is not None:
                ...
                logger.warning("  n_group_lo/hi not found — E_hp_pen will have no bands "
                               "(setting coord_lambda=0 to suppress)")
            ...
            if _rho_grp_fits is not None:
                ...
                for _k, _fit in enumerate(_rho_grp_fits):
                    ...
                    logger.info("    k=%d (%s): ρ*(L)=%.3f - %.3f·exp(-L/%.1f)",
                                _k, _gname,
                                _fit.get("fit_a", 0), _fit.get("fit_b", 0),
                                _fit.get("fit_c", 1))
            else:
            ...
            if _rho_grp_sigma is not None:
                ...
                logger.info("  rho_group_sigma [5]: %s",
                            ", ".join(f"{v:.3f}" for v in _rho_grp_sigma))
            else:
            ...
            if _rho_grp_lo is not None and _rho_grp_hi is not None:
                ...
                logger.info("  rho_group_lo [5]: %s",
                            ", ".join(f"{v:.3f}" for v in _rho_grp_lo))
                logger.info("  rho_group_hi [5]: %s",
                            ", ".join(f"{v:.3f}" for v in _rho_grp_hi))
            else:
                logger.warning("  rho_group_lo/hi not found — E_rho_pen will derive "
                               "bounds from rho_star ± 1.35·sigma")
            ...
            logger.warning("coord_n_star_file not found: %s — packing will use defaults",
                           coord_n_star_file)
...
def verify_model_terms(model: torch.nn.Module, phase: str) -> bool:
    ...
    logger.debug(
        ...
        has_local, has_repulsion, has_secondary, has_packing,
    )
