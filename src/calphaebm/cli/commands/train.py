...
def run(args):
    ...
    if args.resume:
        ...
        if ckpt_path.lower() == "auto":
            ...
            if os.path.exists(phase_dir):
                ...
                if ckpts:
                    steps = [
                        int(f.replace("step", "").replace(".pt", "")) for f in ckpts
                    ]
    ...
    for step in range(start_step + 1, args.steps + 1):
        ...
        if args.phase == "local":
            ...
            B, nphi = phi.shape
            perm = torch.stack(
                [torch.randperm(nphi, device=device) for _ in range(B)], dim=0
            )
            ...
            sim = PyTorchSimulator(model, force_cap=50.0, device=device)
            result = sim.run(
                R0=R_noisy,
                seq=seq,
            ...
            R_neg = result.trajectories[-1].to(device)
            ...
            if args.repulsion_mode == "fixed":
                ...
                logger.info(
                    f"Step {step}: min_dist={min_dist:.3f}, gate_rep={new_gate:.3f}"
                )
                ...
                # Get positive pairs
                r_pos, j = topk_nonbonded_pairs(
                    coords, K=repmod.K, exclude=repmod.exclude
                )
                B, L, K = j.shape
                ...
                rho = repmod.rho(seq)
                loss = (
                    loss + args.rep_radius_reg * ((rho - repmod.rho_base) ** 2).mean()
                )
