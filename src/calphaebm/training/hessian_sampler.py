...
class HessianNegativeSampler:
    ...
    def compute_hessian(
        ...
        logger.info(
            ...
            L,
            h_theta.min().item(), h_theta.max().item(), h_theta.mean().item(),
            h_phi.min().item(), h_phi.max().item(), h_phi.mean().item(),
    ...
    def sample(
        ...
        for T in temps:
            ...
            for _ in range(per_temp):
                ...
                R_pert = nerf_reconstruct(
                    theta_pert, phi_pert, anchor, bond=CA_BOND
                )[0]  # (L, 3)

                negatives.append(HarmonicNegative(
                    R=R_pert,
                    d2_harm=d2_harm,
    ...
    def check_anharmonicity(
        ...
        logger.info(
            "  Anharmonicity: γ=%.3f ± %.3f  (median %.3f)  "
            "sub-harmonic(<0.5)=%.0f%%  super(>2.0)=%.0f%%",
            summary["gamma_mean"], summary["gamma_std"],
    ...
    def generate_full(
        ...
        if check_anharmonicity:
            anharm_summary = self.check_anharmonicity(
                negatives, seq, lengths, hessian.E_native
            )
...
def hessian_curvature_report(
    ...
    lines.append(f"    θ: mean={h_theta.mean():.2f}  min={h_theta.min():.2f}  "
                 f"max={h_theta.max():.2f}  n_soft={int((h_theta < 1.0).sum())}")
    lines.append(f"    φ: mean={h_phi.mean():.2f}  min={h_phi.min():.2f}  "
    ...
    labels = [f"θ_{i+1}" for i in range(len(h_theta))] + \
             [f"φ_{i+1}" for i in range(len(h_phi))]
