...
Target = 1/N E/res where N = number of subterms (auto-detected from model).
...
def _count_subterms(model) -> int:
    """Count total energy subterms from model architecture.

    Returns the FULL count of subterms that will exist in the complete
    model, even if some modules aren't built yet (e.g., during Phase 2
    only local + repulsion are active).
...
def run_repulsion_phase(
    ...
    # Auto-detect number of subterms from model architecture
    n_subterms = _count_subterms(model)
    target_per_subterm = 1.0 / n_subterms
