...
from calphaebm.training.losses.force_balance import force_balance_ic_loss, force_balance_ic_diagnostics
from calphaebm.training.losses.local_geogap_loss import local_geogap_ic_loss, local_geogap_ic_diagnostics
...
from calphaebm.training.losses.elt_losses import elt_funnel_loss, elt_frustration_loss
...
def _setup_optimizer_with_frozen_gates(
    trainer, config, ramp_active: bool, ramp_steps: int, gate_lr_mult: float = 1.0,
    ...
    logger.debug(
        ...
        len(scalar_params), len(frozen_scalar_params), len(main_params), len(gate_params),
    )
...
def run_full_phase(trainer, config, train_loader, val_loader=None, native_structures=None, resume=None):
    ...
    if use_diff_sigma:
        logger.debug(
            ...
            sigma_min_theta, sigma_max_theta, sigma_min_phi, sigma_max_phi,
        )
    ...
    if use_force_balance:
        logger.debug(
            ...
            lambda_fb, fb_sigma_theta, fb_sigma_phi, fb_clash_phi_frac,
        )

    # Geogap (IC)
    lambda_geogap         = float(getattr(config, "lambda_geogap", 0.0) or 0.0)
    geogap_margin         = float(getattr(config, "geogap_margin", 2.0) or 2.0)
    geogap_theta_sigma    = float(getattr(config, "geogap_theta_sigma",   getattr(config, "geogap_angle_sigma",    0.10)) or 0.10)
    geogap_phi_sigma      = float(getattr(config, "geogap_phi_sigma",     getattr(config, "geogap_dihedral_sigma", 0.20)) or 0.20)
    ...
    if use_geogap:
        logger.debug(
            ...
            lambda_geogap, geogap_margin, geogap_theta_sigma, geogap_phi_sigma, geogap_frac_perturbed,
        )
    ...
    if use_pack_contrastive:
        if pack_contrastive_mode == "continuous":
            logger.debug(
                ...
                lambda_pack_contrastive, pack_contrastive_T_base,
            )
        else:
            logger.debug(
                "Packing contrastive loss: lambda=%.3f, margin=%.2f",
                lambda_pack_contrastive, pack_contrastive_margin,
            )
    ...
    if use_balance:
        logger.debug(
            ...
            lambda_balance, balance_r, balance_r, balance_r, balance_r_term, balance_r_term, balance_r_term,
        )
    ...
    if use_discrim:
        logger.info(
            ...
            lambda_discrim, discrim_every, discrim_sigma_min, discrim_sigma_max, discrim_mode,
        )
    ...
    if use_basin:
        if basin_mode == "continuous":
            logger.debug(
                ...
                lambda_basin, basin_T_base,
            )
        else:
            logger.debug(
                "Secondary basin loss: lambda=%.3f, margin=%.2f",
                lambda_basin, basin_margin,
            )
    ...
    if use_native_gap:
        if native_mode == "continuous":
            logger.debug(
                ...
                lambda_native, native_T_base, native_sigma_min, native_sigma_max,
            )
        else:
            logger.debug(
                "Native gap loss: lambda=%.3f, margin=%.2f, sigma=[%.2f, %.2f] rad",
                lambda_native, native_margin, native_sigma_min, native_sigma_max,
            )
    ...
    if use_elt:
        logger.debug(
            ...
            lambda_funnel, funnel_T, lambda_zscore, target_zscore,
            lambda_gap_elt, gap_margin,
            lambda_frustration, frustration_T,
            elt_every, funnel_n_decoys, funnel_sigma_min, funnel_sigma_max,
        )
    ...
    if use_native_depth:
        logger.info(
            "Native depth loss: lambda=%.3f, target=%.2f E/res, "
            "loss=exp(clamp(E_native - target, max=5))",
            lambda_native_depth, target_native_depth,
        )
    ...
    if use_rg_loss:
        logger.info(
            "Rg contrastive loss: lambda=%.3f, α~U(%.2f,%.2f), "
            "loss=softplus(E_native - E_scaled)",
            lambda_rg, rg_alpha_min, rg_alpha_max,
        )
    ...
    if lambda_hb_beta_floor > 0:
        logger.info("Lambda hb_beta floor: %.3f (softplus raw floor=%.3f)",
                     lambda_hb_beta_floor, math.log(math.exp(lambda_hb_beta_floor) - 1))
    ...
    if disable_subterms:
        ...
        for name in disable_subterms:
            ...
            if param.is_leaf:
                ...
                logger.info("Disabled subterm '%s': %s.%s = %.4f (λ=%.6f, frozen)",
                             name, mod_path, attr, zero_val, effective)
            else:
                ...
                for cand in candidates:
                    ...
                    if raw_param is not None and isinstance(raw_param, torch.Tensor) and raw_param.is_leaf:
                        ...
                        logger.info("Disabled subterm '%s': %s.%s (via raw param %s) = -100.0 (λ=%.6f, frozen)",
                                     name, mod_path, attr, cand, effective)
                        found_raw = True
                        break
                if not found_raw:
                    ...
                    if matched:
                        ...
                        logger.info("Disabled subterm '%s': zeroed+froze %d params in %s: %s",
                                     name, len(matched), mod_path, matched)
                        found_raw = True
                if not found_raw:
                    logger.error("Disable subterm '%s': FAILED — no leaf parameter found in %s. "
                                  "Available params: %s", name, mod_path,
                    ...
                    if mlp is not None:
                        ...
                        for pname, pparam in mlp.named_parameters():
                            if pparam.is_leaf:
                                ...
                                zeroed.append(pname)
                        if zeroed:
                            logger.info("Disabled subterm '%s': zeroed+froze %d MLP params in %s.%s: %s",
                                         name, len(zeroed), mlp_mod_path, mlp_attr, zeroed)
        if _disabled_params:
            logger.info("Disabled %d subterms: %s", len(_disabled_params),
                         [n for n, _, _ in _disabled_params])
    ...
    if ramp_active and start_step >= ramp_steps and trainer.optimizer is not None and len(trainer.optimizer.param_groups) > 1:
        gates_unfrozen = True
    ...
    if use_basin:
        try:
            _basin_loss_fn = SecondaryBasinLoss(
                trainer.model.secondary, margin=basin_margin,
                mode=basin_mode, T_base=basin_T_base,
            )
    ...
    for phase_step in range(start_step + 1, n_steps + 1):
        ...
        try:
            ...
            if use_force_balance and not skip_step and phase_step % fb_diag_every == 0:
                try:
                    diag = force_balance_ic_diagnostics(
                        trainer.model, R.detach(), seq.detach(),
                        sigma_theta=fb_sigma_theta,
                        sigma_phi=fb_sigma_phi,
                        sigma_min=sigma_min,
                        sigma_max=sigma_max,
                    ...
                    logger.info(
                        ...
                        phase_step, sc.get("local", 0),
                        _fmt("repulsion"), _fmt("secondary"), _fmt("packing"),
                    )
                except Exception as e:
            ...
            if use_rg_loss and not skip_step and loss is not None:
                try:
                    loss_rg = rg_contrastive_loss(
                        trainer.model, R, seq, lengths=lengths,
                        alpha_min=rg_alpha_min, alpha_max=rg_alpha_max,
                    )
            ...
            if use_balance and not skip_step and loss is not None:
                try:
                    loss_bal, _balance_absmeans, _balance_term_absmeans = energy_balance_loss(
                        trainer.model, R, seq, r=balance_r, r_term=balance_r_term,
                        lengths=lengths,
            ...
            if trainer.optimizer and (loss is not None) and loss.requires_grad and not skip_step:
                ...
                for name, p in trainer.model.named_parameters():
                    ...
                    if not torch.isfinite(p.grad).all():
                        ...
                        logger.error("NaN/Inf grad: %s %s nan=%d inf=%d finite_max=%.3e", name, tuple(p.shape), n_nan, n_inf, g_max)

                if nan_grads:
                    bad_grad_streak += 1
                    logger.error("First bad param: %s %s (streak=%d/%d)", bad_name, bad_shape, bad_grad_streak, bad_grad_streak_max)
                    torch.save(
                        {"name": bad_name, "shape": bad_shape, "phase_step": phase_step,
                         "loss": float(trainer.current_loss), "R": R.detach().cpu(), "seq": seq.detach().cpu()},
            ...
            if not skip_step:
                ...
                if ramp_active and ramp_steps > 0:
                    ...
                    spike_threshold = spike_threshold_start + (spike_threshold_end - spike_threshold_start) * ramp_progress
                ...
                if max_force > spike_threshold:
                    ...
                    if ramp_active and phase_step <= ramp_steps and consecutive_spikes >= 3 and not ramp_paused:
                        logger.warning("%d consecutive spikes - pausing ramp for %d steps", consecutive_spikes, PAUSE_DURATION)
                        ramp_paused   = True
                        pause_counter = 0
                else:
                    consecutive_spikes = 0
        ...
        if phase_step % 200 == 0 and not skip_step:
            try:
                ...
                with torch.no_grad():
                    ...
                    _triu = torch.triu(torch.ones(L_diag, L_diag, device=R_detached.device, dtype=torch.bool), diagonal=1)
                    ...
                    if _nb_dists.numel() > 0:
                        ...
                        _precomputed["safety"] = {"exclude": _exclude, "min_dist": float("inf"),
                                                   "frac_below_40": 0.0, "frac_below_45": 0.0}
                ...
                trainer.diagnostic_logger.log_step_block(
                    phase_step=phase_step, n_steps=n_steps, loss=trainer.current_loss,
                    lr=current_lr, R=R_detached, seq=seq_detached, forces=forces,
                    force_cap=50.0, geogap_diag=_geogap_diag, geogap_margin=geogap_margin,
            ...
            if use_discrim and _loss_discrim_val is not None and _discrim_diag:
                ...
                logger.info(
                    ...
                    _loss_discrim_val, lambda_discrim, lambda_discrim * _loss_discrim_val, gaps_str,
                )

            if use_basin and _basin_loss_fn is not None:
                try:
                    secondary_basin_diagnostics(
                        trainer.model, R_detached, seq_detached, _basin_loss_fn,
                        lengths=lengths,
                    )
                except Exception as e:
        ...
        if phase_step % 50 == 0:
            ...
            if _loss_fb_val       is not None: parts.append(f"fb={_loss_fb_val:.4f}(x{lambda_fb}={lambda_fb*_loss_fb_val:.4f})")
            if _loss_gap_val      is not None: parts.append(f"gap={_loss_gap_val:.4f}(x{lambda_geogap}={lambda_geogap*_loss_gap_val:.4f})")
            if _loss_pack_c_val   is not None: parts.append(f"pack_c={_loss_pack_c_val:.4f}(x{lambda_pack_contrastive}={lambda_pack_contrastive*_loss_pack_c_val:.4f})")
            if _loss_balance_val  is not None: parts.append(f"balance={_loss_balance_val:.4f}(x{lambda_balance}={lambda_balance*_loss_balance_val:.4f})")
            if _loss_native_val   is not None: parts.append(f"native={_loss_native_val:.4f}(x{lambda_native}={lambda_native*_loss_native_val:.4f})")
            if _loss_basin_val    is not None: parts.append(f"basin={_loss_basin_val:.4f}(x{lambda_basin}={lambda_basin*_loss_basin_val:.4f})")
            if _loss_funnel_val   is not None: parts.append(f"funnel={_loss_funnel_val:.4f}(x{lambda_funnel}={lambda_funnel*_loss_funnel_val:.4f})")
            if _loss_zscore_val   is not None: parts.append(f"zscore={_loss_zscore_val:.4f}(x{lambda_zscore}={lambda_zscore*_loss_zscore_val:.4f})")
            if _loss_elt_gap_val  is not None: parts.append(f"elt_gap={_loss_elt_gap_val:.4f}(x{lambda_gap_elt}={lambda_gap_elt*_loss_elt_gap_val:.4f})")
            if _loss_frust_val    is not None: parts.append(f"frust={_loss_frust_val:.4f}(x{lambda_frustration}={lambda_frustration*_loss_frust_val:.4f})")
            if _loss_native_depth_val is not None: parts.append(f"depth={_loss_native_depth_val:.4f}(x{lambda_native_depth}={lambda_native_depth*_loss_native_depth_val:.4f})")
            if _loss_discrim_val is not None: parts.append(f"discrim={_loss_discrim_val:.4f}(x{lambda_discrim}={lambda_discrim*_loss_discrim_val:.4f})")
            if _loss_rg_val is not None: parts.append(f"rg={_loss_rg_val:.4f}(x{lambda_rg}={lambda_rg*_loss_rg_val:.4f})")
            loss_str = " ".join(parts)
            flags = []
            if freeze_gates_steps > 0 and phase_step <= freeze_gates_steps: flags.append("gates=frozen")
            if spike_counter > 0:       flags.append(f"spikes={spike_counter}")
            if bad_grad_streak > 0:     flags.append(f"badgrad={bad_grad_streak}")
            ...
            logger.info(
                ...
                (f"  {loss_str}" if loss_str else ""), flag_str,
            )
        ...
        if val_loader and validate_every > 0 and phase_step % validate_every == 0:
            ...
            if math.isfinite(val_metrics.composite_score):
                if trainer._check_early_stopping(val_metrics.composite_score, config):
                    ...
                    break
                if (trainer.best_composite_score is None) or (val_metrics.composite_score < trainer.best_composite_score):
    ...
    logger.info(
        "Phase %s complete%s", config.name,
