...
from calphaebm.data.pdb_parse import MIN_BOND_LENGTH, MAX_BOND_LENGTH
...
class GenerationValidator(BaseValidator):
    ...
    def _generate_structure(
        ...
        for b in range(B):
            ...
            worker_args.append((
                model_cpu, R_b, seq_b, len_b, n_real,
                self.base_step_size, beta, self.force_cap, self.n_steps,
            ))
    ...
    def validate(
        ...
        logger.info(
            ...
            max_samples, self.n_steps, self.base_step_size, self.langevin_beta,
        )
        ...
        for batch_idx, (R_native, seq, pdb_ids, chain_ids, lengths) in enumerate(val_loader):
            ...
            try:
                (R_gen, success, spike_stats, avg_clip, max_force_preclip,
                 failure_reason, rmsd_traj, energy_init, energy_final,
                 per_struct_deltas, per_struct_rmsf_batch) = (
                    self._generate_structure(R_init, seq, temperature=temperature,
                                             R_ref=R_native, lengths=lengths)
                )
                ...
                if not success:
                    ...
                    failure_counts[
                        failure_reason if failure_reason in failure_counts else "exception"
                    ] += B
                    continue
        ...
        force_result = {
            ...
            "p99_max_force_preclip": float(np.quantile(max_force_arr, 0.99))      if max_force_arr.size > 1
        ...
        if all_theta.size > 0:
            rama_corr = float(compute_ramachandran_correlation(
                torch.tensor(all_theta, device="cpu"),
                torch.tensor(all_phi,   device="cpu"),
            ))
            dphi_corr = float(compute_delta_phi_correlation(
                torch.tensor(all_phi, device="cpu"),
            ))
        else:
            rama_corr = 0.0
        ...
        traj_summary = sorted(
            [(step, float(np.mean(rmsds))) for step, rmsds in traj_accum.items()]
        )
        ...
        logger.info(
            ...
            generation_calls, n_structures_attempted, n_structures_success, generation_failures,
        )
        logger.info(
            "  Spike calls: force=%d/%d (%.2f%%), clip=%d/%d (%.2f%%)",
            spike_calls_force, generation_calls, 100 * spike_calls_force / denom_calls,
            spike_calls_clip,  generation_calls, 100 * spike_calls_clip  / denom_calls,
        )
        ...
        if mean_energy_delta is not None:
            ...
            if energy_deltas_per_struct:
                logger.info(
                    ...
                    len(energy_deltas_per_struct), ed_p05, ed_p50, ed_p95, 100 * ed_neg_frac,
                )
        if traj_summary:
        ...
        logger.info(
            ...
            mean_rmsd, std_rmsd, p05_rmsd, p95_rmsd, max_rmsd,
        )
        logger.info(
            "  dRMSD: mean=%.3f ± %.3f Å  p05=%.3f  p95=%.3f",
            mean_drmsd, std_drmsd, p05_drmsd, p95_drmsd,
        )
        logger.info(
            "  Q (native contacts): mean=%.3f ± %.3f  p05=%.3f  p95=%.3f",
            mean_q, std_q, p05_q, p95_q,
        )
        if rmsf_arr.size:
            logger.info(
                "  RMSF:  mean=%.3f ± %.3f Å",
        ...
        logger.info(
            ...
            bond_mean, bond_std, bond_rmsd, min_bond, max_bond,
        )
