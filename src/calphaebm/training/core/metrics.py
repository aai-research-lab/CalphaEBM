...
def compute_ramachandran_correlation(
    theta: torch.Tensor, 
    phi: torch.Tensor,
    ref_path: str = "analysis/backbone_geometry/data/figure_3a_histogram.npy"
) -> float:
    ...
    try:
        ...
        current_hist, _, _ = np.histogram2d(
            theta_deg, phi_deg,
...
def compute_delta_phi_correlation(
    phi: torch.Tensor,
    ref_dir: str = "analysis/backbone_geometry/data"
) -> float:
