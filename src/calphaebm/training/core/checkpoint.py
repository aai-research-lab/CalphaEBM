...
def _compare_critical_hparams(
    cur_critical: Dict[str, Any],
    sav_critical: Dict[str, Any]
) -> Tuple[bool, List[str]]:
...
def _compute_group_key(
    param_info: List[Tuple[str, Tuple[int, ...]]],
    critical_hparams: Dict[str, Any]
) -> str:
    ...
    json_safe_info = [
        [name, list(shape)]  # shape as list, not tuple
        for name, shape in sorted_info
    ]
    ...
    key_dict = {
        'param_info': json_safe_info,
        'critical_hparams': json_safe_hparams
    }
...
def _tunable_key(sig: Dict[str, Any]) -> tuple:
    ...
    return (_normalize_value(lr) if lr is not None else 0.0,
            _normalize_value(wd) if wd is not None else 0.0)


def _get_group_signature(
    group: Dict[str, Any],
    name_map: Dict[torch.nn.Parameter, str]
) -> Dict[str, Any]:
    ...
    signature = {
        ...
        'tunable_hparams': {
            h: _normalize_value(group.get(h))
            for h in TUNABLE_HPARAMS
            if h in group
        },
    }
...
def _verify_optimizer_compatibility(
    optimizer: torch.optim.Optimizer,
    model: torch.nn.Module,
    saved_signatures: Dict[str, List[Dict[str, Any]]]
) -> bool:
    ...
    for key, cur_groups in current_signatures.items():
        ...
        for i, (cur_sig, sav_sig) in enumerate(zip(cur_groups_sorted, sav_groups_sorted)):
            ...
            compatible, mismatches = _compare_critical_hparams(
                cur_sig['critical_hparams'],
                sav_sig['critical_hparams']
            )
            if not compatible:
            ...
            for hparam in TUNABLE_HPARAMS:
                ...
                if cur_val != sav_val:
                    logger.warning(f"Tunable hyperparameter '{hparam}' mismatch in group {key}[{i}]: "
                                  f"{cur_val} vs {sav_val}")
...
def save_checkpoint(
    ...
    if model is not None and optimizer is not None:
        try:
            ...
            logger.debug(f"Generated optimizer signature with {sum(len(g) for g in optimizer_signature.values())} groups")
...
def _handle_old_signature_format(ckpt: Dict) -> Optional[Dict[str, List[Dict[str, Any]]]]:
    ...
    if sig_version in ["2.3", "2.4"]:
        ...
        logger.warning(f"Old optimizer signature format {sig_version} detected. "
...
def load_checkpoint(path: str, model, optimizer=None, scheduler=None,
                   device=torch.device('cpu'), load_optimizer=True,
                   strict=False):
    ...
    if load_optimizer and optimizer is not None and "optimizer_state" in ckpt:
        ...
        if optimizer_signature is not None:
            ...
            logger.warning("Old checkpoint format (no compatible optimizer signature). Attempting to load optimizer state...")
            try:
                optimizer.load_state_dict(ckpt["optimizer_state"])
...
def find_latest_checkpoint(phase_dir: str) -> Optional[str]:
    ...
    checkpoints = [f for f in os.listdir(phase_dir) 
                  if f.startswith("step") and f.endswith(".pt") and "best" not in f]
    if not checkpoints:
        return None
