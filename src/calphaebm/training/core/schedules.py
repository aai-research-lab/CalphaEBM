...
def get_lr(initial_lr: float, step: int, total_steps: int, 
           schedule: Optional[str], lr_final: Optional[float]) -> float:
...
def apply_gate_schedule(model, phase_step: int, total_steps: int,
                        gate_schedule: Dict[str, List[float]]) -> Dict[str, float]:
