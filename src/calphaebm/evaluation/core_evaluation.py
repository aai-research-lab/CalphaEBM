...
class _v6_model_args:  # noqa: N801
    ...
    # Nonbonded
    repulsion_K = 64;            repulsion_exclude = 3
    repulsion_r_on = 8.0;        repulsion_r_cut = 10.0
    packing_r_on = 8.0;          packing_r_cut = 10.0
    # Packing
    packing_short_gate_on = 4.5; packing_short_gate_off = 5.0
    packing_rbf_centers = [5.5, 7.0, 9.0]; packing_rbf_width = 1.0
    packing_max_dist = 12.0;     packing_init_from = "log_oe"
    packing_normalize_by_length = True
    packing_debug_scale = False; packing_debug_every = 200
    packing_geom_calibration = None
    # Rg / coord (v6)
    coord_lambda = 1.0;          packing_rg_lambda = 1.0
    packing_rg_r0 = 2.0;         packing_rg_nu = 0.38
    packing_rg_dead_zone = 0.30
    packing_rg_m = 1.0;          packing_rg_alpha = 3.0
    packing_coord_m = 1.0;       packing_coord_alpha = 2.0
    packing_rho_lambda = 1.0
    packing_rho_m = 1.0;         packing_rho_alpha = 2.0
    hp_penalty_lambda = 1.0;     rho_penalty_lambda = 1.0
    # Local
    init_theta_theta_weight = 1.0; init_delta_phi_weight = 1.0
    local_window_size = 8;       debug_mode = False
...
def load_model(ckpt_path: Path,
    ...
    model = build_model({"local", "repulsion", "secondary", "packing"},
                        device, _v6_model_args())
    ...
    # Strategy 3: use training checkpoint loader
        ...
        from calphaebm.cli.commands.train.checkpoint import (
            load_checkpoint as _lc,
...
def load_structures(
    ...
    collate = (getattr(ds, "collate_fn", None) or
               getattr(ds.__class__, "collate", None))
    ...
    for batch in loader:
        ...
        lengths   = (batch[4] if len(batch) >= 5
                     else torch.full((R.shape[0],), R.shape[1]))
...
def structures_to_loader(structures: List[Tuple]) -> DataLoader:
    ...
    return DataLoader(_DS(structures), batch_size=16, shuffle=False,
                      collate_fn=_collate)
