...
def find_round_checkpoint(ckpt_dir: Path, prefix: str,
                           round_num: int) -> Optional[Path]:
...
def eval_round(
    ...
    """
    import copy, gc, subprocess, sys, tempfile

    log_fn(f"Round {round_num}: {ckpt_path.name}  "
           f"({len(structures)} proteins × {n_steps} steps, {n_workers} workers)")
    t0 = time.time()
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="eval_round_") as tmpdir:
            ...
            cmd = [
                ...
                "--results-path",    str(raw_results_path),
            ...
            raw_results = torch.load(str(raw_results_path), map_location="cpu",
                                     weights_only=False)
        ...
        def _compute_backbone_js(all_theta, all_phi, refs):
            ...
            if "rama_P" in refs:
                ...
                H_sim, _, _ = np.histogram2d(th_rama, ph_rama,
            ...
            if "phiphi_P" in refs and n_rama >= 2:
                ...
                H_sim, _, _ = np.histogram2d(phi_i, phi_ip1,
                                             bins=[ph_i_edges, ph_ip1_edges])
                H_sim = H_sim + 1e-10
                P_sim = H_sim / H_sim.sum()
            ...
            if "thetatheta_P" in refs and n_rama >= 2:
                ...
                H_sim, _, _ = np.histogram2d(theta_i, theta_ip1,
                                             bins=[th_i_edges, th_ip1_edges])
                H_sim = H_sim + 1e-10
                P_sim = H_sim / H_sim.sum()
        ...
        def _basin_assignment(r):
            ...
            if n < 4:
                return {}
            basin_centers = [
                (92.5, 55.0, "helix"), (117.5, -155.0, "sheet"),
                (92.5, 85.0, "PPII"), (122.5, -105.0, "turn"),
            ]
        ...
        for i, r in enumerate(raw_results):
            ...
            rec["pdb_id"] = r.get("pdb_id",
                structures[i][2] if i < len(structures) else "?")
            rec["chain_id"] = r.get("chain_id",
                structures[i][3] if i < len(structures) else "?")
        ...
        log_fn(f"  Q={summary['q_mean']:.3f}  RMSD={summary['rmsd_mean']:.2f}  "
               f"ΔE={summary['e_delta_mean']:+.3f}  "
               f"Q_af={summary['q_af_mean']:.1f}%  dRMSD_af={summary['drmsd_af_mean']:.1f}%")
        log_fn(f"  accept={summary['accept_pct_mean']:.1f}%  "
        ...
        if summary.get("e_local_mean") is not None:
            log_fn(f"  E/res: local={summary['e_local_mean']:.3f}  "
        ...
        if summary.get("rama_helix_frac_mean"):
            log_fn(f"  Rama: helix={summary['rama_helix_frac_mean']:.1%}  "
        ...
        if fes:
            np.savez(_fes_path(results_dir, round_num),
                     Q_edges=fes["Q_edges"], RMSD_edges=fes["RMSD_edges"],
                     F=fes["F"], Q_native=1.0, RMSD_native=0.0,
                     n_samples=fes["n_samples"], round_num=round_num, beta=beta)
...
def run_watch(args) -> int:
    ...
    while current <= args.max_rounds:
        ...
        if done.exists():
            ...
            current += 1;  n_done += 1;  continue

        ckpt = find_round_checkpoint(ckpt_dir, args.ckpt_prefix, current)
        if ckpt is None:
            _wlog(f"Round {current}: waiting for checkpoint "
